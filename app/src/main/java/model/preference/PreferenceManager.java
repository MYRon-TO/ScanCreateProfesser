...
public class PreferenceManager {
     ...
     */
    public void setPreferenceIsDarkMode(boolean isDarkMode) {
        lock.writeLock().lock();
        try {
     ...
     */
    public void setPreferencePathToSaveNote(Uri path) {
        lock.writeLock().lock();
        try {
            sharedPreferences.edit().putString("pathToSaveNote", path.toString()).apply();
        } catch (Exception ignored) {
        } finally {
            lock.writeLock().unlock();
        }
    ...
    public String getPreferenceTextRecognizerModel() {
        lock.readLock().lock();
        try {
            return sharedPreferences.getString("textRecognizerModel", "Latin");
        } finally {
            lock.readLock().unlock();
        }
    }
